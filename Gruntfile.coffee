...
module.exports = ( grunt ) ->
    ...
    grunt.initConfig
        ...
        meta :
            version : "<%= pkg.version %>"
            core_banner : "// ExtM (Extende Marionette)\n" + "// ----------------------------------\n" + "// v<%= pkg.version %>\n" + "//\n" + "// Copyright (c)<%= grunt.template.today(\"yyyy\") %> Suraj Air, Ajency.in.\n" + "// Distributed under MIT license\n" + "//\n" + "// http://ajency.in\n" + "\n"
    ...
    grunt.registerMultiTask "unwrap", "Unwrap UMD", ->
        done = @async()
        timesLeft = 0
        @files.forEach ( file ) ->
            file.src.forEach ( src ) ->
                timesLeft++
                unwrap path.resolve( __dirname, src ), ( err, content ) ->
    ...
    grunt.registerTask "lint", "Lints our sources", [
        "lintspaces"
        "jshint"
    ]
    grunt.registerTask "dev", "Auto-lints while writing code.", [
        "lint"
        "unwrap"
        "preprocess:bundle"
        "jasmine:marionette"
    ...
    grunt.registerTask "build", "Build all three versions of the library.", [
        "clean:lib"
        "lint"
        "unwrap"
        "preprocess"
